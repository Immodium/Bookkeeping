...
export const CompanyHeader: React.FC<CompanyHeaderProps> = ({ companyLogo, onLogoUpload }) => {
  ...
  return (
    <div className="flex items-center space-x-4">
      {(displayLogo || onLogoUpload) && (
        <div className={`w-16 h-16 bg-muted rounded-lg flex items-center justify-center relative overflow-hidden ${onLogoUpload ? 'border-2 border-dashed border-border hover:border-primary cursor-pointer' : ''}`}>
          {displayLogo ? (
